// ============================================
        ...
        // ============================================
        function initChat() {
            // Check for URL parameters (from Shanti modal)
            const urlParams = new URLSearchParams(window.location.search);
            const tool = urlParams.get('tool');
            
            if (tool === 'breathing' || tool === 'grounding') {
                // Skip to layer 4 and show the requested tool
                goToLayer(4);
                setTimeout(() => {
                    const toolElement = document.querySelector(`button[onclick="showRegulationTool('${tool}')"]`);
                    if (toolElement) {
                        toolElement.click();
                    }
                }, 100);
                addBotMessage("Let's take a moment to find some calm together.");
            } else if (currentLayer === 1) {
                addBotMessage("Welcome to Raksha. I'm here to walk alongside you through whatever you're experiencing. Let's take this one step at a time.");
            }
        }
        ...
        function showRegulationTool(tool) {
            ...
            const buttons = document.querySelectorAll('.regulation-tab');
            buttons.forEach(btn => {
                if (btn.textContent.toLowerCase().includes(tool.toLowerCase())) {
                    btn.style.background = 'var(--saffron-muted)';
